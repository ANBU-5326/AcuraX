      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-semibold">
                  ...
                  <span className="text-violet-400">{Math.round(threshold * 100)}%</span>
                </div>
                <input 
                  type="range"
                  min="0.4"
                  ...
                  disabled={searching}
                  className="w-full h-1 bg-slate-900 rounded-lg appearance-none cursor-pointer accent-violet-600 border border-slate-800"
                />
              ...
              <Button type="submit" className="w-full h-10 mt-2" loading={searching}>
        ...
        <div className="lg:col-span-3 space-y-4">
          {searching ? (
            ...
            <Card className="p-12 text-center flex flex-col items-center justify-center min-h-[250px] border-dashed border-slate-800" glass>
            ...
            <Card className="p-12 text-center flex flex-col items-center justify-center min-h-[250px] border-dashed border-slate-800" glass>
              ...
              <p className="text-slate-300 font-bold">Semantic inspector</p>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                {results.map((res) => {
                  ...
                  return (
                    <Card key={res.id} className="relative overflow-hidden hover:border-slate-700/60" glass>
                      <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <div className="flex items-center gap-2">
                          <Highlighter className="h-4.5 w-4.5 text-violet-400" />
                          <CardTitle className="text-sm font-bold">{res.title}</CardTitle>
                        </div>
                        <Badge 
                          className={`text-[9px] ${
                            res.score >= 0.9 
                      ...
                      <CardContent className="space-y-3">
                        <p className="text-xs text-slate-400 leading-relaxed bg-slate-950/40 p-3 rounded-lg border border-slate-900/60 font-mono">
                          {res.snippet}
                        </p>
